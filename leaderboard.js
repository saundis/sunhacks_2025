    ...
    `;
        ...
        window.reset = function patchedReset() {
            try {
                if (
                    window.game &&
                    Number.isFinite(window.game.score) &&
                    window.game.score > 0
                ) {
        ...
        $remember.addEventListener('click', () =>
            setName($nameIn.value.trim() || 'YOU')
        );
        $save.addEventListener('click', () => {
            const score =
                window.game && window.game.score
                    ? Math.floor(window.game.score)
                    : 0;
        ...
        $export.addEventListener('click', () => {
            ...
            a.href = url;
            a.download = 'hazard_runner_leaderboard.json';
        ...
        window.addEventListener('beforeunload', () => {
            try {
                if (window.game && window.game.score > 0) {
                    ...
                    board.push({
                        name: getName() || 'AUTO',
                        score: Math.floor(window.game.score),
                        at: Date.now(),
                    });
